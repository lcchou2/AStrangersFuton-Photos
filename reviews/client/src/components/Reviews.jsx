...
class Reviews extends React.Component {
  ...
  render() {
    ...
    var listOfReviews = this.props.reviews.slice(start, end).map((review, index) =>
      <div key={index}>
        <div>
          <img className="rev-avatar" src={review.profile}/>
          <div className="rev-profileInfo">
            <div className="rev-username">{review.name}</div>
            <br/>
            <div className="rev-date">{review.month} {review.year}</div>
          </div>
          <Flag />
        </div>
        <br/>
    ...
    if (this.props.reviews.length === 0) {
      return (<div>
          <span className="rev-noReviews">None of our guests have mentioned "<strong>{this.props.value}</strong>"</span>
          <span className="rev-backToReviews" onClick={this.props.backToReviews}><u>Back to all reviews</u></span>
        </div>
      );
    } else if (this.props.reviews.length < this.props.allReviews.length) {
      return (<div>
        <span className="rev-someReviews">{this.props.reviews.length} guests have mentioned "<strong>{this.props.value}</strong>"</span>
        <span className="rev-backToReviews" onClick={this.props.backToReviews}><u>Back to all reviews</u></span>
        <hr/>
